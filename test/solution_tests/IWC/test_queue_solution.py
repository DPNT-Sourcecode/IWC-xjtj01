...
def test_bank_statement_skip_4() -> None:
    run_queue([
        ...
        call_enqueue(provider="companies_house", user_id=3, timestamp=iso_ts(delta_minutes=1)).expect(3),
        call_enqueue(provider="id_verification", user_id=3, timestamp=iso_ts(delta_minutes=7)).expect(4),
        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("bank_statements", 2),
        call_dequeue().expect("companies_house", 3),
...
def test_bank_statement_skip_5() -> None:
    run_queue([
        call_enqueue(provider="bank_statements", user_id=1, timestamp=iso_ts(delta_minutes=0)).expect(1),
